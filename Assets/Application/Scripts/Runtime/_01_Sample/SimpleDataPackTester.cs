...
{
	public partial class SimpleDataPackTester : CancelableTask
	{
		public SimpleDataPackTester( ExMonoBehaviour owner ) : base( owner ){}

		public async UniTask RunDebug<T>() where T : class //,new()
		{
			Debug.Log( "<color=#7F7FFF>==================== SimpleDataPack 検証 ====================</color>" ) ;
...
//			Debug.Log( "P10:" + o1.P10 ) ;
			...
			Debug.Log( "<color=#00FF00>[SimpleDataPack]シリアライズ時間(１回目) = " + ( Time.realtimeSinceStartup - t1 ) + "</color>" ) ;

			if( data1 == null )
			{
				Debug.LogWarning( "[SimpleDataPack]シリアライズに失敗しました(１回目)" ) ;
			}
			else
			{
				Debug.Log( "<color=#00FF7F>[SimpledataPack]★★★シリアライズに成功しました(１回目) : Size = " + data1.Length + "★★★</color>" ) ;
...
//			SimpleDataPack.Clear() ;
			...
			Debug.Log( "<color=#00FFFF>[SimpleDataPack]デシリアライズ時間(１回目) = " + ( Time.realtimeSinceStartup - t1 ) + "</color>" ) ;

			if( sd1 == null )
			{
				Debug.Log( "<color=#FF7F00>[SimpleDataPack]デシリアライズに失敗しました(１回目)</color>" ) ;
			}
			else
			{
				Debug.Log( "<color=#00FFFF>[SimpleDataPack]★★★デシリアライズに成功しました(１回目)★★★</color>" ) ;
