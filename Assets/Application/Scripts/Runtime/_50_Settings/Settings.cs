...
namespace DSW
{
	...
	{
		[Header( "システムバージョン名" )]
		public string	SystemVersionName = "0.1.13" ;
		...
		public EndPointData[] WebAPI_EndPoints = new EndPointData[]
		{
			new EndPointData()
			{
				...
				DisplayName = m_EndPointNames[ EndPointNames.Experiment		], Path = "https://api-experiment.pj-dbs.net/api/",
				Description = "システムバージョンアップ対応用の環境です"
			},
			new EndPointData()
			{
				Name		= EndPointNames.Development,
				DisplayName = m_EndPointNames[ EndPointNames.Development	], Path = "https://api-default.pj-dbs.net/api/",
				Description = "基本の開発用の環境です"
			},
			new EndPointData()
			{
				Name        = EndPointNames.Debug,
				DisplayName = m_EndPointNames[ EndPointNames.Debug			], Path = "https://api-testing.pj-dbs.net/api/",
				Description = "デバッグ専用の環境です",
			},
			new EndPointData()
			{
				Name		= EndPointNames.Staging,
				DisplayName = m_EndPointNames[ EndPointNames.Staging		], Path = "https://api-stg.pj-dbs.net/api/",
				Description = "L社やKT社の確認用の環境です"
			},
			new EndPointData()
			{
				Name		= EndPointNames.Review,
				DisplayName = m_EndPointNames[ EndPointNames.Review			], Path = "https://api-cert.pj-dbs.net/api/",
				Description = "Appleのアプリ審査用の環境です"
			},
			new EndPointData()
			{
				Name		= EndPointNames.Release,
				DisplayName = m_EndPointNames[ EndPointNames.Release		], Path = "https://api-cbt.pj-beast.net/api/",
