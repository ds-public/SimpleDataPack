...
public partial class SimpleDataPack
{
	...
	public class ArrayNDPrimitiveAdapter<T> : IAdapter
	{
		...
		private readonly Type	m_PrimitiveType ;
		...
		public ArrayNDPrimitiveAdapter( int rank )
		{
			...
			m_PrimitiveType			= typeof( T ) ;
			var primitiveTypeCode	= Type.GetTypeCode( m_PrimitiveType ) ;
		...
		public System.Object Deserialize( ByteStream reader )
		{
			...
			// アレイ生成
			var elements = Array.CreateInstance( typeof( T ), m_Lengths ) ;
	...
	public class ArrayNDPrimitiveNAdapter<T> : IAdapter
	{
		...
		private readonly Type	m_NullablePrimitiveType ;
		private readonly Type	m_PrimitiveType ;
		...
		public ArrayNDPrimitiveNAdapter( int rank )
		{
			...
			m_NullablePrimitiveType		= typeof( T ) ;
			m_PrimitiveType				= Nullable.GetUnderlyingType( m_NullablePrimitiveType ) ;
			var primitiveTypeCode		= Type.GetTypeCode( m_PrimitiveType ) ;
	...
	/// </summary>
	/// <typeparam name="T"></typeparam>
	public class ArrayNDPrimitiveVersatileAdapter : IAdapter
	{
		public readonly int		Rank ;
		...
		/// コンストラクタ
		/// </summary>
		public ArrayNDPrimitiveVersatileAdapter( Type objectType, Type elementType, int rank )
		{
			// 次元数を保存する
			Rank = rank ;
		...
		public System.Object Deserialize( ByteStream reader )
		{
			...
			// アレイ生成
			var elements = Array.CreateInstance( m_PrimitiveType, m_Lengths ) ;
	...
	/// </summary>
	/// <typeparam name="T"></typeparam>
	public class ArrayNDPrimitiveNVersatileAdapter : IAdapter
	{
		public readonly int		Rank ;
		...
		/// コンストラクタ
		/// </summary>
		public ArrayNDPrimitiveNVersatileAdapter( Type objectType, Type elementType, int rank )
		{
			// 次元数を保存する
			Rank = rank ;
			...
			m_NullablePrimitiveType		= elementType ;
			m_PrimitiveType			= Nullable.GetUnderlyingType( m_NullablePrimitiveType ) ;
			var primitiveTypeCode = Type.GetTypeCode( m_PrimitiveType ) ;
				...
				case TypeCode.Boolean :
					SetValue = ( Array elements, int length, ByteStream writer ) =>
					{
						...
						for( index  = 0 ; index <  length ; index ++ )
						{
							...
							if( value == null ){ writer.PutByte( 0 ) ; }else{ writer.PutBooleanT( ( System.Boolean )value ) ; }
							Increase() ;
						}
					} ;
					GetValue = ( Array elements, int length, ByteStream reader ) =>
				...
				case TypeCode.Char :
					SetValue = ( Array elements, int length, ByteStream writer ) =>
					{
						...
						for( index  = 0 ; index <  length ; index ++ )
						{
							// System.Enum? → System.ValueType? キャストはやってはいけない(危険・IL2CPPでは動かない)
							value = elements.GetValue( m_Indices ) ;
							if( value == null ){ writer.PutByte( 0 ) ; }else{ writer.PutCharT( ( System.Char )value ) ; }
							Increase() ;
						}
					} ;
					GetValue = ( Array elements, int length, ByteStream reader ) =>
				...
				case TypeCode.Single :
					SetValue = ( Array elements, int length, ByteStream writer ) =>
					{
						...
						for( index  = 0 ; index <  length ; index ++ )
						{
							// System.Enum? → System.ValueType? キャストはやってはいけない(危険・IL2CPPでは動かない)
							value = elements.GetValue( m_Indices ) ;
							if( value == null ){ writer.PutByte( 0 ) ; }else{ writer.PutSingleT( ( System.Single )value ) ; }
							Increase() ;
						}
					} ;
					GetValue = ( Array elements, int length, ByteStream reader ) =>
				...
				case TypeCode.Double :
					SetValue = ( Array elements, int length, ByteStream writer ) =>
					{
						...
						for( index  = 0 ; index <  length ; index ++ )
						{
							// System.Enum? → System.ValueType? キャストはやってはいけない(危険・IL2CPPでは動かない)
							value = elements.GetValue( m_Indices ) ;
							if( value == null ){ writer.PutByte( 0 ) ; }else{ writer.PutDoubleT( ( System.Double )value ) ; }
							Increase() ;
						}
					} ;
					GetValue = ( Array elements, int length, ByteStream reader ) =>
				...
				case TypeCode.Decimal :
					SetValue = ( Array elements, int length, ByteStream writer ) =>
					{
						...
						for( index  = 0 ; index <  length ; index ++ )
						{
							// System.Enum? → System.ValueType? キャストはやってはいけない(危険・IL2CPPでは動かない)
							value = elements.GetValue( m_Indices ) ;
							if( value == null ){ writer.PutByte( 0 ) ; }else{ writer.PutDecimalT( ( System.Decimal )value ) ; }
							Increase() ;
						}
					} ;
					GetValue = ( Array elements, int length, ByteStream reader ) =>
				...
				case TypeCode.DateTime :
					SetValue = ( Array elements, int length, ByteStream writer ) =>
					{
						...
						for( index  = 0 ; index <  length ; index ++ )
						{
							// System.Enum? → System.ValueType? キャストはやってはいけない(危険・IL2CPPでは動かない)
							value = elements.GetValue( m_Indices ) ;
							if( value == null ){ writer.PutByte( 0 ) ; }else{ writer.PutDateTimeT( ( System.DateTime )value ) ; }
							Increase() ;
						}
					} ;
					GetValue = ( Array elements, int length, ByteStream reader ) =>
		...
		public System.Object Deserialize( ByteStream reader )
		{
			...
			// アレイ生成
			var elements = Array.CreateInstance( m_NullablePrimitiveType, m_Lengths ) ;
